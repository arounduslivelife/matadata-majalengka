function openSidebar() {
    ...
    function handleSearch() {
        ...
        if (currentMode === 'sirup') {
            ...
            // CROSS-YEAR SEARCH: Search all years in allAudits
            const matchPackages = allAudits.filter(p => p.nama.toLowerCase().includes(query)).slice(0, 8);
            matchPackages.forEach(p => {
                ...
                html += `<div class="result-item" onclick="selectPackageFromSearch('${p.id}', '${p.kecamatan}', ${p.tahun})">
                            ...
                            <div style="font-size: 0.7rem; color: #fbbf24; font-weight: 600; margin-bottom: 4px;">🏗️ ${p.vendor || 'Belum Ada Pemenang'}</div>
                            <div style="font-size:0.6rem; opacity:0.5;">🔍 ${p.satker || 'Satker'}</div>
                         </div>`;
            });
    ...
    window.showVendorIntelligence = function(vendorName) {
        if (!vendorName || vendorName === 'PL') return;
        
        // Pindah ke Sidebar Audit secara paksa
        const btnAudit = document.getElementById('btn-audit') || document.querySelector('[onclick*="sidebar-audit"]');
        if(btnAudit) btnAudit.click();

        const reportContainer = document.getElementById('audit-district-report');
        if(reportContainer) {
            reportContainer.style.display = 'block';
            reportContainer.scrollIntoView({ behavior: 'smooth' });
        }
        
        const titleEl = document.getElementById('audit-report-title');
        if(titleEl) {
            titleEl.innerText = "Vendor Intelligence";
            titleEl.style.color = "#60a5fa";
        }

        let html = `
            <div style="background: rgba(59,130,246,0.1); padding: 15px; border-radius: 12px; border: 1px solid rgba(59,130,246,0.2); margin-bottom: 20px;">
                <div style="font-size: 0.6rem; opacity: 0.6; text-transform: uppercase;">Profil Penyedia Terpilih</div>
                <div style="font-size: 1.2rem; font-weight: 800; color: #60a5fa; margin: 5px 0;">${vendorName}</div>
                <div style="font-size: 0.75rem; opacity: 0.7;">Analisa portofolio digital infrastruktur Majalengka.</div>
            </div>
            <div style="max-height: 400px; overflow-y: auto; padding-right:5px;" class="custom-scroll">
        `;

        let projects = [];
        if (infraData.school) {
            infraData.school.features.forEach(f => {
                if (f.properties.vendor === vendorName) projects.push({...f.properties, type: '🏫 Sekolah'});
            });
        }
        if (infraData.bridge) {
            infraData.bridge.features.forEach(f => {
                f.properties.paket.forEach(pkg => {
                    if (pkg.vendor === vendorName) projects.push({...pkg, type: '🌉 Jembatan', lokasi: f.properties.lokasi});
                });
            });
        }

        if (projects.length === 0) {
            html += `<div style="text-align:center; padding:20px; opacity:0.5;">Data portfolio tidak ditemukan untuk filter tahun ini.</div>`;
        } else {
            let totalValue = 0;
            projects.forEach(p => {
                totalValue += (parseFloat(p.pagu) || 0);
                html += `
                    <div style="background: rgba(255,255,255,0.03); padding: 12px; border-radius: 10px; margin-bottom: 10px; border: 1px solid rgba(255,255,255,0.05);">
                        <div style="display: flex; justify-content: space-between; margin-bottom: 5px;">
                            <span style="font-size: 0.6rem; color: #60a5fa; font-weight: 800;">${p.type}</span>
                            <span style="font-size: 0.6rem; opacity: 0.5;">T.A ${p.tahun}</span>
                        </div>
                        <div style="font-size: 0.8rem; font-weight: 600; margin-bottom: 8px;">${p.nama}</div>
                        <div style="font-size: 0.75rem; color: #10b981; font-weight: 800;">${formatPaguJS(p.pagu)}</div>
                    </div>
                `;
            });
            ...
            html = `
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px; margin-bottom: 20px;">
                    <div style="background: rgba(255,255,255,0.05); padding: 10px; border-radius: 10px; text-align: center; border: 1px solid rgba(255,255,255,0.1);">
                        <div style="font-size: 0.6rem; opacity: 0.5;">TOTAL PAKET</div>
                        <div style="font-size: 1.2rem; font-weight: 800;">${projects.length}</div>
                    </div>
                    <div style="background: rgba(255,255,255,0.05); padding: 10px; border-radius: 10px; text-align: center; border: 1px solid rgba(255,255,255,0.1);">
                        <div style="font-size: 0.6rem; opacity: 0.5;">TOTAL NILAI</div>
                        <div style="font-size: 1rem; font-weight: 800; color: #10b981;">Rp ${(totalValue/1000000000).toFixed(1)}M</div>
                    </div>
                </div>
            ` + html;
        }
        ...
        html += `</div>`;
        const bodyEl = document.getElementById('audit-report-body');
