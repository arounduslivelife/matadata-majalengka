...
db.exec("DROP TABLE IF EXISTS realization_2026");
db.exec(`CREATE TABLE realization_2026 (
...
const insert = db.prepare(`INSERT INTO realization_2026 
...
const importData = async () => {
    ...
    for await (const line of rl) {
        ...
        insert.run(satker, kodePacket, rup, sourceDana, vendor, method, type, packageName, status, total, pdn, kecamatan, lat, lng);
        count++;
    }

    console.log(`Ingested ${count} realization records.`);
};
